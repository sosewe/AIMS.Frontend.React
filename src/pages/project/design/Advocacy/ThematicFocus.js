...
const ThematicFocus = ({ id }) => {
  ...
  const mutation = useMutation({ mutationFn: saveAdvocacyThematicFocus });
  ...
  const { refetch } = useQuery(
    ["deleteAdvocacyThematicFocus", thematicFocusId],
    deleteAdvocacyThematicFocus,
    { enabled: false }
  );

  const handleDeleteAdvocacyThematicFocus = async () => {
    await refetch();
    setOpen(false);
    await queryClient.invalidateQueries([
                  ...
                  <Grid container item spacing={2}>
                    <Grid item md={12}>
                      ...
                      >
                        ...
                        <DialogActions>
                          <Button
                            onClick={handleDeleteAdvocacyThematicFocus}
                            color="primary"
                          >
                            Yes
                          </Button>
