...
const KMDocumentsUploadForm = (props) => {
  ...
  const {
    ...
    isLoading: isLoadingTechnicalAssistanceDocument,
  } = useQuery(
    ["getTechnicalAssistanceDocumentByTechnicalAssistanceId", id],
    getTechnicalAssistanceDocumentByTechnicalAssistanceId,
    {
      enabled: !!id,
    }
  ...
  const mutationDocument = useMutation({
    mutationFn: newTechnicalAssistanceDocument,
  });
  ...
  useEffect(() => {
    function setCurrentFormValues() {
      if (
        !isLoadingTechnicalAssistanceDocument &&
        TechnicalAssistanceDocumentData.data &&
        TechnicalAssistanceDocumentData.data.length > 0
      ) {
        const allDocuments = [];
        for (const item of TechnicalAssistanceDocumentData.data) {
          const document = {
            documentInitials: item.documentInitials,
            documentName: item.documentName,
            documentDescription: item.documentDescription,
  ...
  }, [TechnicalAssistanceDocumentData, isLoadingTechnicalAssistanceDocument]);
