...
import {
  newLearningMonitoringDocument,
  getLearningMonitoringDocumentByResearchId,
} from "../../../../api/learning-document";
...
const KMDocumentsUploadForm = ({ id, processLevelItemId }) => {
  ...
  const { data: LearningDocumentData, isLoading: isLoadingLearningDocument } =
    useQuery(
      ["getLearningMonitoringDocumentByResearchId", id],
      getLearningMonitoringDocumentByResearchId,
      {
        enabled: !!id,
      }
  ...
  const mutationDocument = useMutation({
    mutationFn: newLearningMonitoringDocument,
  });
    ...
    onSubmit: async (values) => {
      try {
        ...
        for (const item of documentsList) {
          ...
          researchDocuments.push(document);
        }
        await mutationDocument.mutateAsync(researchDocuments);
  ...
  useEffect(() => {
    function setCurrentFormValues() {
      if (
        !isLoadingLearningDocument &&
        LearningDocumentData.data &&
        LearningDocumentData.data.length > 0
      ) {
        const allDocuments = [];
        for (const item of LearningDocumentData.data) {
          const document = {
            documentInitials: item.documentInitials,
            documentName: item.documentName,
            documentDescription: item.documentDescription,
